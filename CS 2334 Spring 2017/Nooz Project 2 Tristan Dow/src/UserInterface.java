...
public class UserInterface {
	 ...
	 */
	public static String queryNewsMakerName(String query) {
	...
	public static String createListOfNewspaperStoriesForNewsmaker(NewsMaker newsMaker) {
		...
		NewsStoryList list = newsMaker.getNewsStories();
	 ...
	 * @return Returns a String
	 */
	private static String convertToOutputFormat(NewsStory newsStory) {
		LocalDate date = newsStory.getDate();
		String month = date.getMonth().toString().toLowerCase();
		month = Character.toUpperCase(month.charAt(0)) + month.substring(1);
		int day = date.getDayOfMonth();
		int year = date.getYear();
		return month + " " + day + ", " + year + "; " + newsStory.getPublisherName() + "; " + newsStory.getWordCount()
				+ " words; " + newsStory.getTopic();
	}

	/**
	 * Prints a list of NewsStories along with the NewsMaker they are about
	 * 
	 * @param listOfStories
